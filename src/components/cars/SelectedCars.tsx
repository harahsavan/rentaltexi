...
const SelectedCars: React.FC<SelectedCarsProps> = ({ filteredData }) => {
  ...
  const handleSelect = (booking: Booking) => {
    const carWithDetails = {
      cityFrom: booking.cityFrom,
      cityTo: booking.cityTo,
    ...
    // dispatch(addCar(carWithDetails));
    ...
    alert(`${booking.selectedCar} selected!`);
    setShowDetails(true);
        ...
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            <div
              ...
              <div>
                {userFormData?.cityFrom && (
                  <div>FORM: {userFormData?.cityFrom}</div>
                )}
                {userFormData?.cityTo && (
                  <div>TO: {userFormData?.cityTo}</div>
                )}
          ...
          <table className="table-auto w-full border-collapse border border-gray-300">
            ...
            <tbody>
              {filteredData.map((booking, index: number) => (
                <tr key={index}>
